...
msmoneyrep = {
    ...
    'OFX':{
        'skip':lambda row,grid: False,
        'BANKID':lambda row,grid: "CapitalOne Platinum MasterCard",
        'ACCTID':lambda row,grid: 0,
        'DTPOSTED':lambda row,grid: toOFXDate(fromCSVCol(row,grid,'Transaction Date')),
        'TRNAMT':lambda row,grid: fromCSVCol(row,grid,'Billing Amount'),
...
def capitalone_memo(row,grid):
    memo=fromCSVCol(row,grid,' Merchant Town ') # sometimes None
    cat=fromCSVCol(row,grid,' Merchant County ')
    cls=fromCSVCol(row,grid,' Merchant Postcode ')
    ref=fromCSVCol(row,grid,' Reference Number ')
    sicmcc = fromCSVCol(row,grid, ' SICMCC Code').replace('"','')
    return "%s %s %s Ref: %s SICMCC: %s" % ( memo, cat, cls, ref, sicmcc )

def capitalone_toOFXDate(date):
    return datetime.strptime(date,'%d/%m/%Y').strftime('%Y%m%d')

def capitalone_toQIFDate(date):
    return datetime.strptime(date,'%d/%m/%Y').strftime('%m/%d/%Y')

def capitalone_amount(row,grid):
    amount = fromCSVCol(row,grid,' Billing Amount').decode('latin1')
    if amount[0]=="-":
        return amount[2:] # Discard '-£' and flip sign
    else:
        return "-"+amount[1:] # Discard '£' and add negative sign
...
capitalone = {
    'OFX':{
        'skip':lambda row,grid: False,
        'BANKID':lambda row,grid: 'CapitalOne',
        'ACCTID':lambda row,grid: 'My Account',
        'DTPOSTED':lambda row,grid: capitalone_toOFXDate(fromCSVCol(row,grid,'Transaction Date')),
        'TRNAMT':lambda row,grid: capitalone_amount(row,grid),
        'FITID':lambda row,grid: row,
        'PAYEE':lambda row,grid: fromCSVCol(row,grid,' Merchant'),
        'MEMO':lambda row,grid: capitalone_memo(row,grid),
        'CURDEF':lambda row,grid: 'GBP',
        'CHECKNUM':lambda row,grid: ''
    },
    'QIF':{
        'split':lambda row,grid:False,
        'Account':lambda row,grid: 'CapitalOne',
        'AccountDscr':lambda row,grid: 'My Account',
        'Date':lambda row,grid: capitalone_toQIFDate(fromCSVCol(row,grid,'Transaction Date')),
        'Payee':lambda row,grid: fromCSVCol(row,grid,' Merchant'),
        'Memo':lambda row,grid: capitalone_memo(row,grid),
        'Category':lambda row,grid:'Unclassified',
        'Class':lambda row,grid:'',
        'Amount':lambda row,grid: capitalone_amount(row,grid),
        'Number':lambda row,grid:''        
    }
}


all_mappings = {'Yodlee':yodlee, 'Credit Union':cu, 'UBS':ubs, 'MS Money Report (CSV)':msmoneyrep, 'CapitalOne':capitalone}
